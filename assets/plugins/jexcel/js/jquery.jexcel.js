  ...
  var methods = {
    ...
    init: function(options) {
      ...
      if ($.fn.jexcel.defaults[id].csv) {
        if (!$.csv) {
          ...
          // Comma as default
          $.fn.jexcel.defaults[id].delimiter =
            $.fn.jexcel.defaults[id].delimiter || ",";
        ...
        $.ajax({
          ...
          success: function(result) {
            // Data
            $.fn.jexcel.defaults[this.id].data = result.data
              ? result.data
              : result;
            // Prepare table
            $.fn.jexcel("prepareTable", this.id);
    ...
    prepareTable: function(id) {
      ...
      for (var i = 0; i < size; i++) {
        ...
        if (!$.fn.jexcel.defaults[id].colWidths[i]) {
          $.fn.jexcel.defaults[id].colWidths[i] =
            $.fn.jexcel.defaults[id].defaultColWidth || "50";
        }
        ...
        if (
          $.fn.jexcel.defaults[id].columns[i].type == "autocomplete" ||
          $.fn.jexcel.defaults[id].columns[i].type == "dropdown"
        ) {
          // if remote content
            ...
            results.push(
              $.ajax({
                ...
                success: function(result) {
                  ...
                  $.fn.jexcel.defaults[this.id].columns[
                    this.index
                  ].source = result;
                },
                error: function(result) {
                  console.error(
                    "It was not possible to load the url: " + this.url
                  );
                }
              })
            );
    ...
    createTable: function() {
        ...
        var createNestedHeader = function(nestedInformation) {
          ...
          for (var i = 0; i < nestedInformation.length; i++) {
            ...
            contentRow +=
              ...
              headerClasses +
              '" colspan="' +
              nestedInformation[i].colspan +
              '" align="' +
              nestedInformation[i].align +
              '">' +
              nestedInformation[i].title +
              "</td>";
          }
      ...
      for (var i = 0; i < options.colHeaders.length; i++) {
        ...
        // Display
        var display =
          options.columns[i].type == "hidden" ? ' style="display:none"' : "";
      ...
      // Populate header
      $(thead).append(
        '<tr class="jexcel_headers_nested">' + contentNested + "</tr>"
      );
      ...
      // Append content
      $(tableHeader).append(
        '<colgroup><col width="30">' + contentWidth + "</colgroup>"
      );
      ...
      if ($.fn.jexcel.defaults[id].tableOverflow == true) {
        if ($.fn.jexcel.defaults[id].tableWidth) {
          ...
          $(tableContentContainer).css(
            "max-width",
            $.fn.jexcel.defaults[id].tableWidth
          );
        }
        ...
        if ($.fn.jexcel.defaults[id].tableHeight) {
          ...
          $(tableContentContainer).css(
            "max-height",
            $.fn.jexcel.defaults[id].tableHeight
          );
          ...
          if ($.fn.jexcel.defaults[id].tableWidth) {
            ...
            $(tableHeaderContainer).css(
              "max-width",
              parseInt($.fn.jexcel.defaults[id].tableWidth) - 17
            );
          }
        } else {
          if ($.fn.jexcel.defaults[id].tableWidth) {
      ...
      if (!$(".jexcel_corner").length) {
        ...
        $(ads).html(
          '<a href="https://github.com/paulhodel/jexcel">jExcel Spreadsheet</a>'
        );
        ...
        $.fn.jexcel.contextMenuControls = function(e) {
          if ($($.fn.jexcel.selectedCell).hasClass("edition")) {
            ...
            if ($.fn.jexcel.current) {
              ...
              var isJexcel = $(e.target).parents(".jexcel").length
                ? true
                : false;
              ...
              if (isJexcel) {
                ...
                if (o) {
                  ...
                  // Custom context menu
                  if (
                    typeof $.fn.jexcel.defaults[$.fn.jexcel.current]
                      .contextMenu == "function"
                  ) {
                    contextMenuContent = $.fn.jexcel.defaults[
                      $.fn.jexcel.current
                    ].contextMenu(o[0], o[1], e);
                  } else {
                    // Default context menu for the columns
                    contextMenuContent = $.fn.jexcel(
                      "contextMenu",
                      o[0],
                      o[1],
                      e
              ...
              if ($(jexcelTable).length > 0) {
                ...
                if (
                  ...
                  if (o) {
                    ...
                    if (
                      $.fn.jexcel.selectedHeader &&
                      (e.shiftKey || e.ctrlKey)
                    ) {
                      var d = $($.fn.jexcel.selectedHeader)
                        .prop("id")
                        .split("-");
                    } else {
                    ...
                    if ($(e.target).outerWidth() - e.offsetX < 8) {
                      if (
                        $.fn.jexcel.defaults[$.fn.jexcel.current]
                          .columnResize == true
                      ) {
                        // Resize helper
                        ...
                        // Border indication
                        $("#" + $.fn.jexcel.current + " .c" + o[1]).addClass(
                          "resizing"
                        );
                        $("#" + $.fn.jexcel.current + " .h" + o[1]).addClass(
                          "resizing"
                        );
                        $("#" + $.fn.jexcel.current + " #col-" + o[1]).addClass(
                          "resizing"
                        );
                      ...
                      // Get cell objects
                      var o1 = $("#" + $.fn.jexcel.current).find(
                        "#" + o[1] + "-0"
                      );
                      var o2 = $("#" + $.fn.jexcel.current).find(
                        "#" +
                          d[1] +
                          "-" +
                          parseInt(
                            $.fn.jexcel.defaults[$.fn.jexcel.current].data
                              .length - 1
                          )
                      );
                ...
                if ($(e.target).parents(".jexcel-content").length) {
                  ...
                  if ($(e.target).is(".jexcel_label")) {
                    if (
                      $.fn.jexcel.defaults[$.fn.jexcel.current].rowDrag ==
                        true &&
                      $(e.target).outerWidth() - e.offsetX < 8
                    ) {
                      // Reset selection
                      $("#" + $.fn.jexcel.current).jexcel("resetSelection");
                      // Mark which row we are dragging
                      $.fn.jexcel.dragRowFrom = $(e.target)
                        .parent()
                        .prop("id");
                      $.fn.jexcel.dragRowOver = $(e.target)
                        .parent()
                        .prop("id");
                      ...
                      if (
                        $.fn.jexcel.selectedRow &&
                        (e.shiftKey || e.ctrlKey)
                      ) {
                        // Updade selection multi columns
                        var d = $($.fn.jexcel.selectedRow)
                          .prop("id")
                          .split("-");
                      } else {
                        // Update selection single column
                      ...
                      var o2 = $("#" + $.fn.jexcel.current).find(
                        "#" +
                          parseInt(
                            $.fn.jexcel.defaults[$.fn.jexcel.current].columns
                              .length - 1
                          ) +
                          "-" +
                          d[1]
                ...
                if (
                  !$(e.target).parents(".jexcel, .jexcel_contextmenu").length
                ) {
        ...
        $.fn.jexcel.mouseUpControls = function(e) {
          ...
          if (e.target.id == "jexcel_arrow") {
            ...
            if ($.fn.jexcel.resizeColumn) {
              ...
              // New width
              var newWidth = $(
                "#" + $.fn.jexcel.current + " .jexcel-header col"
              )
                .eq(parseInt($.fn.jexcel.resizeColumn.column) + 1)
                .prop("width");
              ...
              // On resize
              if (
                typeof $.fn.jexcel.defaults[$.fn.jexcel.current].onresize ==
                "function"
              ) {
                $.fn.jexcel.defaults[$.fn.jexcel.current].onresize(
                  $(this),
                  $.fn.jexcel.resizeColumn.column,
                  newWidth,
                  $.fn.jexcel.resizeColumn.width
                );
              }
            ...
            // Data to be copied
            var selection = $("#" + $.fn.jexcel.current).find(
              "tbody td.selection"
            );
            ...
            if ($(selection).length > 0) {
              ...
              var o = $(selection[0])
                .prop("id")
                .split("-");
              var d = $(selection[selection.length - 1])
                .prop("id")
                .split("-");
              ...
              $(selection).removeClass(
                "selection selection-left selection-right selection-top selection-bottom"
              );
            }
          }
          ...
          if ($.fn.jexcel.dragRowFrom) {
            ...
            $("#" + $.fn.jexcel.current + " #" + $.fn.jexcel.dragRowOver)
              .parent()
              .find("td")
        ...
        $.fn.jexcel.doubleClickControls = function(e) {
          ...
          if ($.fn.jexcel.current) {
            if ($.fn.jexcel.defaults[$.fn.jexcel.current].editable == true) {
              ...
              if (e.target.id == "jexcel_corner") {
                ...
                if (typeof selection == "object") {
                  ...
                  o[1] = parseInt(d[1]) + 1;
                  d[1] = parseInt(
                    $.fn.jexcel.defaults[$.fn.jexcel.current].data.length
                  );
                  // Do copy
                  $("#" + $.fn.jexcel.current).jexcel("copyData", o, d);
                }
            ...
            if (
              $.fn.jexcel.defaults[$.fn.jexcel.current].columnSorting == true
            ) {
              // Header found
              if (
                $(e.target)
        ...
        $.fn.jexcel.mouseMoveControls = function(e) {
          if ($.fn.jexcel.current) {
            if (
              $.fn.jexcel.defaults[$.fn.jexcel.current].columnResize == true
            ) {
              // Resizing is ongoing
              if ($.fn.jexcel.resizeColumn) {
                ...
                if ($.fn.jexcel.resizeColumn.width + width > 0) {
                  ...
                  $("#" + $.fn.jexcel.current)
                    .find(".jexcel-header col")
                    .eq(parseInt($.fn.jexcel.resizeColumn.column) + 1)
                    .prop("width", tempWidth);
                }
              } else {
                // Header found
                if (
                ...
                if ($(e.target).is(".jexcel_label")) {
                  ...
                  if ($.fn.jexcel.ignoreEvents != true) {
                    if ($.fn.jexcel.defaults[id].onmouseover) {
                      if (
                        typeof $.fn.jexcel.defaults[id].onmouseover ==
                        "function"
                      ) {
                        $.fn.jexcel.defaults[id].onmouseover(
                          $("#" + $.fn.jexcel.current),
                          $(e.target)
                        );
                      }
        ...
        $.fn.jexcel.pasteControls = function(e) {
          if ($.fn.jexcel.selectedCell) {
            if (!$($.fn.jexcel.selectedCell).hasClass("edition")) {
              if (e.originalEvent) {
                if (
                  $.fn.jexcel.defaults[$.fn.jexcel.current].editable == true
                ) {
                  $("#" + $.fn.jexcel.current).jexcel(
                    "paste",
                    $.fn.jexcel.selectedCell,
        ...
        $.fn.jexcel.keyDownControls = function(e) {
          if ($.fn.jexcel.current) {
            ...
            // Get current cell
            if ($.fn.jexcel.selectedCell) {
              columnId = $($.fn.jexcel.selectedCell)
                .prop("id")
                .split("-");
              ...
              if (e.which == 37) {
                ...
                if (!$($.fn.jexcel.selectedCell).hasClass("edition")) {
                  ...
                  if (e.ctrlKey) {
                    ...
                    cell = $($.fn.jexcel.selectedCell)
                      ...
                      .next()
                      .find(
                        "#" + columnId[0] + "-" + (parseInt(columnId[1]) + 1)
                      );
                  }
                  e.preventDefault();
                }
                ...
                if ($($.fn.jexcel.selectedCell).hasClass("edition")) {
                  ...
                  if (
                    ...
                    if (
                      ...
                      var editorTextarea = $($.fn.jexcel.selectedCell).find(
                        ".editor"
                      );
                      var editorValue = $(editorTextarea).val();
                      var editorIndexOf = $(editorTextarea).prop(
                        "selectionStart"
                      );
                      editorValue =
                ...
                if (!$($.fn.jexcel.selectedCell).hasClass("edition")) {
                  if (
                    $.fn.jexcel.defaults[$.fn.jexcel.current].allowInsertRow ==
                    true
                  ) {
                    if (
                      $.fn.jexcel.defaults[$.fn.jexcel.current]
                ...
                if (
                  $.fn.jexcel.defaults[$.fn.jexcel.current].allowInsertColumn ==
                  true
                ) {
                  if (
                    $.fn.jexcel.defaults[$.fn.jexcel.current]
                    ...
                    if (
                      columnId[0] ==
                      $.fn.jexcel.defaults[$.fn.jexcel.current].data[0].length -
                        1
                    ) {
                  ...
                  if (!$($.fn.jexcel.selectedCell).hasClass("edition")) {
                    if ($.fn.jexcel.selectedRow) {
                      if (
                        ...
                        if (
                          confirm("Are you sure to delete the selected rows?")
                        ) {
                      ...
                      if (
                        ...
                        if (
                          ...
                          var columns = $("#" + $.fn.jexcel.current)
                            .find("thead.jexcel_label")
                            .find(".selected");
                          $("#" + $.fn.jexcel.current).jexcel(
                            "deleteColumn",
                            parseInt(
                      ...
                      $("#" + $.fn.jexcel.current).jexcel(
                        "setValue",
                        $("#" + $.fn.jexcel.current).find(".highlight"),
                        ""
                      );
                    }
                  }
                ...
                if (e.metaKey && !e.shiftKey && !e.ctrlKey) {
                        ...
                        if (!$($.fn.jexcel.selectedCell).hasClass("edition")) {
                          ...
                          if (e.keyCode == 32) {
                            ...
                            if (
                              ...
                              var checkboxCurrentVal = $(
                                "#" + $.fn.jexcel.current
                              ).jexcel("getValue", $.fn.jexcel.selectedCell);
                              $("#" + $.fn.jexcel.current).jexcel(
                                "setValue",
                                $.fn.jexcel.selectedCell,
                                checkboxCurrentVal == 1 ? false : true
                              );
                              e.preventDefault();
                            } else {
                              $("#" + $.fn.jexcel.current).jexcel(
                  ...
                  if (!$($.fn.jexcel.selectedCell).hasClass("edition")) {
                    if (e.which == 65) {
                      ...
                      // Ctrl + X
                      if (
                        $.fn.jexcel.defaults[$.fn.jexcel.current].editable ==
                        true
                      ) {
        ...
        if (typeof options.cells == "function") {
          ...
          $.each(cells, function(k, v) {
            ...
            options.table(
              ...
              coords[0],
              coords[1],
              $.fn.jexcel.defaults[id].data[coords[1]][coords[0]],
              $.fn.jexcel("getColumnNameFromId", [coords[0], coords[1]])
            );
          });
        }
      }
    ...
    openEditor: function(cell, empty, e) {
      ...
      if ($(cell).hasClass("readonly") == true) {
        ...
        if (options.columns[position[0]].editor) {
          ...
          // Native functions
          if (
            options.columns[position[0]].type == "checkbox" ||
            options.columns[position[0]].type == "radio" ||
            options.columns[position[0]].type == "hidden"
          ) {
            // Get value
            ...
            options.columns[position[0]].type == "dropdown" ||
            options.columns[position[0]].type == "autocomplete"
          ) {
            // Keep the current value
            $(cell).addClass("edition");
            ...
            if (typeof options.columns[position[0]].filter == "function") {
              var source = options.columns[position[0]].filter(
                $(this),
                $(cell),
                position[0],
                position[1],
                options.columns[position[0]].source
              );
            } else {
            ...
            if ($.fn.jdropdown) {
              ...
              $(editor).jdropdown({
                ...
                autocomplete:
                  options.columns[position[0]].autocomplete ||
                  options.columns[position[0]].type == "autocomplete"
                    ? true
                    : false,
                opened: true,
                value: options.columns[position[0]].multiple
                  ? value.split(";")
                  : value,
                onclose: function() {
            ...
            if (
              ...
              options.columns[position[0]].wordWrap == true
            ) {
            ...
            if (options.columns[position[0]].mask) {
              if (!$.fn.masked) {
                console.error(
                  "Jexcel: it was not possible to load the mask plugin."
                );
              } else {
                $(editor).mask(
                  options.columns[position[0]].mask,
                  options.columns[position[0]].options
                );
              }
            }
      ...
      if ($(cell).length) {
        ...
        if (options.columns[position[0]].editor) {
          ...
          if (
            ...
            options.columns[position[0]].type == "dropdown" ||
            options.columns[position[0]].type == "autocomplete" ||
            options.columns[position[0]].type == "calendar"
          ) {
            // Get value
            value = $(cell)
              .find("input")
    ...
    updateCell: function(v, force) {
      ...
      if ($(v.cell).hasClass("readonly") == true && force == false) {
          ...
          if (
            ...
            $(v.cell).html(
              '<input type="hidden" value="' +
                ...
                '">' +
                val +
                '<span class="jexcel_arrow"><span id="jexcel_arrow"></span></span>'
            );
          } else if (options.columns[position[0]].type == "calendar") {
            val = "";
            if (value != "undefined") {
              ...
              date = $.fn.jcalendar(
                "fromFormatted",
                value,
                options.columns[position[0]].options.format
              );
              if (date) {
                value = date;
              }
        ...
        window[i] = new $.fn.jexcel.factory(
          i,
          $.fn.jexcel.defaults[id].values[i]
        );
    ...
    updateSelection: function(o, d, origin) {
      ...
      if (o && d) {
        ...
        for (var i = borderLeft; i <= borderRight; i++) {
          ...
          $(this)
            .find("#" + i + "-" + borderTop)
            .addClass("highlight-top");
          // Bottom border
          $(this)
            .find("#" + i + "-" + borderBottom)
            .addClass("highlight-bottom");
        }
        ...
        for (var j = borderTop; j <= borderBottom; j++) {
          ...
          $(this)
            .find("#" + borderLeft + "-" + j)
            .addClass("highlight-left");
          // Right border
          $(this)
            .find("#" + borderRight + "-" + j)
            .addClass("highlight-right");
        }
    ...
    updateCornerSelection: function(current) {
      ...
      for (j = py; j <= uy; j++) {
        for (i = px; i <= ux; i++) {
          ...
          $(this)
            .find("#" + i + "-" + py)
            .addClass("selection-top");
          $(this)
            .find("#" + i + "-" + uy)
            .addClass("selection-bottom");
          $(this)
            .find("#" + px + "-" + j)
            .addClass("selection-left");
          $(this)
            .find("#" + ux + "-" + j)
            .addClass("selection-right");
        }
      }
    },
      ...
      if ($(cells).length > 0) {
        ...
        $.each(cells, function(k, v) {
          ...
          records.push({
            ...
            newValue: "",
            oldValue:
              $.fn.jexcel.defaults[$.fn.jexcel.current].data[position[1]][
                position[0]
              ]
          });
        });
    ...
    paste: function(cell, data) {
      ...
      if (position) {
        ...
        if (y + data.length > $.fn.jexcel.defaults[id].data.length) {
          $(this).jexcel(
            "insertRow",
            y + data.length - $.fn.jexcel.defaults[id].data.length
          );
        }
        ...
        if (data[0]) {
          ...
          if (x + row.length > $.fn.jexcel.defaults[id].data[y].length) {
            $(this).jexcel(
              "insertColumn",
              x + row.length - $.fn.jexcel.defaults[id].data[y].length
            );
          }
        }
        ...
        for (j = 0; j < data.length; j++) {
          ...
          for (i = 0; i < row.length; i++) {
            ...
            cell = $(this).find(
              "#" +
                (parseInt(i) + parseInt(x)) +
                "-" +
                (parseInt(j) + parseInt(y))
            );
            ...
            if ($(cell).length > 0) {
              ...
              records.push({
                ...
                newValue: row[i],
                oldValue:
                  $.fn.jexcel.defaults[id].data[parseInt(j) + parseInt(y)][
                    parseInt(i) + parseInt(x)
                  ]
              });
            }
    ...
    parseCSV: function(CSV_string, delimiter) {
      ...
      var pattern = new RegExp( // Delimiters:
        "(\\" +
          delimiter +
          "|\\r?\\n|\\r|^)" +
          // Quoted fields.
    ...
    insertColumn: function(mixed, properties, columnNumber) {
      ...
      if (options.allowInsertColumn == true) {
        ...
        var currentColHeaderClasses = options.colHeaderClasses.splice(
          columnNumber
        );
        ...
        Array.prototype.push.apply(
          options.colHeaderClasses,
          currentColHeaderClasses
        );

        // Visual reference
        if (direction == 1) {
          var referenceHeader = $(this).find(
            ".jexcel-header #col-" + (columnNumber - 1)
          );

          // Add width control
          var referenceWidthHeader = $(this)
            .find(".jexcel-header > table > colgroup > col")
            .eq(columnNumber);
          var referenceWidthContent = $(this)
            .find(".jexcel-content > table > colgroup > col")
            .eq(columnNumber);

          // Adding visual headers
          for (
            var col = numOfColumns + columnNumber - 1;
            col >= columnNumber;
            col--
          ) {
            // Adding width control
            $(referenceWidthHeader).after(
              '<col width="' + options.colWidths[col] + '">'
            );
            $(referenceWidthContent).after(
              '<col width="' + options.colWidths[col] + '">'
            );
            // Adding the header
            ...
            var display =
              options.columns[col].type == "hidden"
                ? ' style="display:none"'
                : "";
            td =
              '<td align="' +
              options.colAlignments[col] +
              '" title="' +
              title +
              '" class="' +
              options.colHeaderClasses[col] +
              '"' +
              display +
              ">" +
              header +
          ...
          for (var row = 0; row < options.data.length; row++) {
            ...
            var currentData = $.fn.jexcel.defaults[id].data[row].splice(
              columnNumber
            );

            // Reference
            var referenceColumn = $(this)
              .find("tbody")
              .find("#row-" + row)
              .find(".c" + (columnNumber - 1));
            ...
            for (
              ...
              // New value
              $.fn.jexcel.defaults[id].data[row][col] = data[row]
                ? data[row]
                : "";
          ...
          var referenceHeader = $(this).find(
            ".jexcel-header #col-" + columnNumber
          );
          ...
          for (
            ...
            col < numOfColumns + columnNumber;
            col++
          ) {
            // Adding width control
            $(referenceWidthHeader).before(
              '<col width="' + options.colWidths[col] + '">'
            );
            $(referenceWidthContent).before(
              '<col width="' + options.colWidths[col] + '">'
            );
            // Adding the header
            var title = options.colHeaders[col];
    ...
    insertRow: function(mixed, rowNumber) {
      ...
      if (options.allowInsertRow == true) {
        ...
        // Add row to the table body
        if (direction == 1) {
          var referenceRow = $(this)
            .find("tbody")
            .find("#row-" + (rowNumber - 1));
        } else {
          var referenceRow = $(this)
            .find("tbody")
            .find("#row-" + rowNumber);
        ...
        var createRow = function() {
          ...
          for (
            var col = 0;
            col < $.fn.jexcel.defaults[id].colHeaders.length;
            col++
          ) {
            // New Data
            $.fn.jexcel.defaults[id].data[row][col] = data[col]
              ? data[col]
              : "";
      ...
      if (options.allowDeleteColumn == true) {
        ...
        if (options.data[0].length > 1) {
          if (parseInt(columnNumber) > -1) {
            ...
            var colHeaders = options.colHeaders.splice(
              columnNumber,
              numOfColumns
            );
            var colWidths = options.colWidths.splice(
              columnNumber,
              numOfColumns
            );
            var colAlignments = options.colAlignments.splice(
              columnNumber,
              numOfColumns
            );
            var colHeaderClasses = options.colHeaderClasses.splice(
              columnNumber,
              numOfColumns
            );
            ...
            for (
              var row = 0;
              row < $.fn.jexcel.defaults[id].data.length;
              row++
            ) {
              // Keep columns data
      ...
      if (options.allowDeleteRow == true) {
        ...
        if (options.data.length > 1) {
          if (parseInt(rowNumber) > -1) {
            ...
            // Remove from source
            $.fn.jexcel.defaults[id].data.splice(
              parseInt(rowNumber),
              numOfRows
            );
    ...
    orderBy: function(column, order) {
      if (column >= 0) {
        ...
        if (order == 1) {
          $(c)
            .find("#col-" + column)
            .append('<span class="arrow-up"></span>')
    ...
    executeFormula: function(cellId) {
      ...
      if (typeof excelFormulaUtilities !== "object") return;
      ...
      if (
        ...
        // Update variables
        var letter =
          $.fn.jexcel("getColumnName", cellId[0]) + (parseInt(cellId[1]) + 1);
        ...
        // Update window
        window[letter] = new $.fn.jexcel.factory(
          letter,
          $.fn.jexcel.defaults[id].values[letter]
        );
        // New cell value
     ...
     * @return null
     */
    download: function(filename, sheetname) {
      var colWidths = $(this).jexcel("getWidth");
      var data = $(this).jexcel("getData");
      data.unshift($(this).jexcel("getHeaders", false));

      /* original data */
      var filename = filename ? filename + ".xlsx" : "report.xlsx";
      ...
      for (var i = 0; i < colWidths.length; i++) {
        wscols.push({ width: colWidths[i] / 5 });
      }
      ws["!cols"] = wscols;
    ...
    setHistory: function(changes, action) {
      if ($.fn.jexcel.ignoreHistory != true) {
        ...
        var history = ($.fn.jexcel.defaults[id].history = $.fn.jexcel.defaults[
          id
    ...
    undo: function() {
      ...
      if ($.fn.jexcel.defaults[id].historyIndex >= 0) {
        ...
        var historyRecord =
          $.fn.jexcel.defaults[id].history[
            $.fn.jexcel.defaults[id].historyIndex--
          ];
        ...
        if (historyRecord.action) {
          ...
          if (historyRecord.action.type == "insertRow") {
            ...
            var numOfRows =
              historyRecord.action.mixed > 0 ? historyRecord.action.mixed : 1;
            ...
            for (var row = rowNumber; row < rowNumber + numOfRows; row++) {
              $(this).jexcel(
                "insertRow",
                historyRecord.action.rowData[row],
                row
              );
            }
            ...
            for (
              ...
              $(this).jexcel(
                ...
                historyRecord.action.columnData[columnIndex],
                {
                  ...
                  colHeaderClasses: [
                    columnProperties.colHeaderClasses[columnIndex]
                  ]
                },
                col
              );
            ...
            $(this).jexcel(
              "moveRow",
              historyRecord.action.to,
              historyRecord.action.fr
            );
          ...
          $(this).jexcel(
            "updateSelection",
            historyRecord.firstSelected,
            historyRecord.lastSelected
          );
    ...
    redo: function() {
      ...
      if (
        $.fn.jexcel.defaults[id].historyIndex <
        $.fn.jexcel.defaults[id].history.length - 1
      ) {
        // History
        var historyRecord =
          $.fn.jexcel.defaults[id].history[
        ...
        if (historyRecord.action) {
          ...
          if (historyRecord.action.type == "insertRow") {
            ...
            $(this).jexcel(
              "insertRow",
              historyRecord.action.mixed,
              historyRecord.action.rowNumber
            );
            ...
            $(this).jexcel(
              "deleteRow",
              historyRecord.action.rowNumber,
              historyRecord.action.numOfRows
            );
            ...
            $(this).jexcel(
              "insertColumn",
              historyRecord.action.mixed,
              historyRecord.action.properties,
              historyRecord.action.columnNumber
            );
            ...
            $(this).jexcel(
              "deleteColumn",
              historyRecord.action.columnNumber,
              historyRecord.action.numOfColumns
            );
    ...
    createCell: function(i, j) {
      ...
      if (options.columns[i].type == "hidden") {
        ...
        if (options.columns[i].readOnly == true) {
          $(td).html(
            '<input type="radio" name="c' + i + '" disabled="disabled">'
          );
        } else {
    ...
    spareCheck: function() {
      ...
      if ($.fn.jexcel.defaults[id].minSpareCols > 0) {
        ...
        lastCol = $.fn.jexcel.defaults[id].data[0]
          ? $.fn.jexcel.defaults[id].data[0].length
          : 0;
    ...
    moveRow: function(o, d) {
      ...
      $.fn.jexcel.defaults[id].data.splice(
        d,
        0,
        $.fn.jexcel.defaults[id].data.splice(o, 1)[0]
      );
    ...
    contextMenu: function(type, number, e) {
      ...
      if (type == "col") {
        if (
          $.fn.jexcel.defaults[$.fn.jexcel.current].allowInsertColumn == true
        ) {
          contextMenuContent +=
            "<a onclick=\"jQuery('#" +
            $.fn.jexcel.current +
            "').jexcel('insertColumn', 1, null, " +
            number +
            ')">Insert a new column<span></span></a>';
        }
        if (
          $.fn.jexcel.defaults[$.fn.jexcel.current].allowDeleteColumn == true
        ) {
          contextMenuContent +=
            "<a onclick=\"jQuery('#" +
            $.fn.jexcel.current +
            "').jexcel('deleteColumn')\">Delete this column<span></span></a>";
        }
        contextMenuContent +=
          "<a onclick=\"jQuery('#" +
          $.fn.jexcel.current +
        ...
        if ($.fn.jexcel.defaults[$.fn.jexcel.current].columnSorting == true) {
          contextMenuContent +=
            ...
            "').jexcel('orderBy', " +
            number +
            ', 0)">Order ascending <span></span></a>';
          contextMenuContent +=
            "<a onclick=\"jQuery('#" +
            $.fn.jexcel.current +
            "').jexcel('orderBy', " +
            number +
            ', 1)">Order descending <span></span></a>';
        }
        contextMenuContent +=
          "<hr><a onclick=\"jQuery('#" +
          $.fn.jexcel.current +
          "').jexcel('copy', true)\">Copy...<span>Ctrl + C</span></a>";
        contextMenuContent +=
          "<a onclick=\"jQuery('#" +
          $.fn.jexcel.current +
          "').jexcel('download')\">Save as...<span>Ctrl + S</span></a>";
        if ($.fn.jexcel.defaults[$.fn.jexcel.current].about) {
          contextMenuContent +=
            "<a onclick=\"alert('" +
            $.fn.jexcel.defaults[$.fn.jexcel.current].about +
            "')\">About<span></span></a>";
        }
      } else {
        ...
        if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowInsertRow == true) {
          contextMenuContent +=
            ...
            "').jexcel('insertRow', 1, " +
            number +
            ')">Insert a new row<span></span></a>';
        }
        if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowDeleteRow == true) {
          contextMenuContent +=
            "<a onclick=\"jQuery('#" +
            $.fn.jexcel.current +
            "').jexcel('deleteRow')\">Delete this row<span></span></a>";
        }
        contextMenuContent += "<hr>";
        if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowComments == true) {
          if (!$($.fn.jexcel.selectedCell).prop("title")) {
            contextMenuContent +=
              "<a onclick=\"jQuery('#" +
              $.fn.jexcel.current +
              "').jexcel('setComments', $.fn.jexcel.selectedCell, prompt('Comments', ''));\">Add comments<span></span></a><hr>";
          } else {
            contextMenuContent +=
              "<a onclick=\"jQuery('#" +
              $.fn.jexcel.current +
              "').jexcel('setComments', $.fn.jexcel.selectedCell, prompt('Comments', $($.fn.jexcel.selectedCell).prop('title')));\">Edit comments<span></span></a>";
            contextMenuContent +=
              "<a onclick=\"jQuery('#" +
              $.fn.jexcel.current +
              "').jexcel('setComments', $.fn.jexcel.selectedCell, '');\">Reset comments<span></span></a><hr>";
          }
        }
        contextMenuContent +=
          "<a onclick=\"jQuery('#" +
      ...
      if (el.tagName != "TABLE") {
        ...
        for (var i = 0; i < options.columns.length; i++) {
          ...
          if (testCalendar) {
            ...
            test == true &&
            Object.keys(pattern[i]).length <=
              parseInt(options.data.length * 0.1)
          ) {
    ...
    destroyAll: function() {
      if (
        $.fn.jexcel.defaults &&
        Object.keys($.fn.jexcel.defaults).length > 0
      ) {
        var keys = Object.keys($.fn.jexcel.defaults);
        // Remove HTML elements
        $(this).html("");
        // Remove HTML controls
        $(".jexcel_corner").remove();
